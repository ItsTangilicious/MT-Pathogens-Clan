...
{
    internal class CardEffectAddTempCardUpgradeToCardsInHandAttuned : CardEffectBase, ICardEffectTipTooltip
    {
        ...
        public override IEnumerator ApplyEffect(CardEffectState cardEffectState, CardEffectParams cardEffectParams)
        {
            ...
            foreach (CardState item in cardEffectParams.cardManager.GetHand())
            {
                ...
                {
                    flag2 = flag2 && traitState.OnCardBeingUpgraded(item, cardEffectState.GetParentCardState(), cardEffectParams.cardManager, cardUpgradeState);
                }
                if (flag2)
                {
                    CardUpgradeState cardUpgradeState2 = new CardUpgradeState();
                    cardUpgradeState2.Setup(cardUpgradeState);
                    cardUpgradeState2.SetAttackDamage((item.GetTotalAttackDamage() *4 ) * item.GetMagicPowerMultiplierFromTraits());
                    cardUpgradeState2.SetAdditionalHeal(cardUpgradeState2.GetAdditionalHeal()* item.GetMagicPowerMultiplierFromTraits());
                    //since there is not GetTotalHeal, I do not know how to make Attuned retroactively apply to upgraded heal spells.
