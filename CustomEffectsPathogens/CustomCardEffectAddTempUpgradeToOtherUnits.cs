...
namespace CustomEffectsPathogens
{
    ...
    public sealed class CustomCardEffectAddTempCardUpgradeToOtherUnits : CardEffectBase, ICardEffectTipTooltip
    {
        ...
        public override IEnumerator ApplyEffect(CardEffectState cardEffectState, CardEffectParams cardEffectParams)
        {
            ...
            {
                if (target == cardEffectParams.selfTarget) { continue; }
                if (cardEffectState.GetParamBool() && target.GetSourceCharacterData() == cardEffectParams.selfTarget.GetSourceCharacterData())
                {
                    //Trainworks.Trainworks.Log(BepInEx.Logging.LogLevel.Info, "Skipped adding upgrade because duplicate unit detected.");
                    continue;
                }
                bool flag = false;
                foreach (CharacterTriggerState activeTrigger in target.GetTriggers())
                {
                    Trainworks.Trainworks.Log(BepInEx.Logging.LogLevel.Info, "Active Trigger: " + activeTrigger.GetTrigger().ToString() + " ParamTrigger: " + cardEffectState.GetParamTrigger());

                    if (activeTrigger.GetTrigger() == cardEffectState.GetParamTrigger())
                    {
                        //Trainworks.Trainworks.Log(BepInEx.Logging.LogLevel.Info, "Skipped adding upgrade because filtered trigger detected.");
                        flag = true;
                        break;
                    }
                }
                ...
                /*
				if (target.GetSpawnerCard() != null && cardEffectState.GetParamCardFilter() != null)
				{ 
					if (!cardEffectState.GetParamCardFilter().FilterCard<CardState>(target.GetSpawnerCard(), null))
					{
                        Trainworks.Trainworks.Log(BepInEx.Logging.LogLevel.Info, "Skipped adding upgrade because card failed the filter check.");
                        continue;
					}
				}
				*/
